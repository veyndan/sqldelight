...
class SelectQueryTypeTest {
  ...
  fun `compatible java types from different columns checks for adapter equivalence`(dialect: DialectPreset) {
    ...
    assertThat(generator.customResultTypeFunction().toString()).isEqualTo(
      ...
      |  kotlin.check(kotlin.collections.setOf(database.childrenAdapter.birthdayAdapter, database.adultsAdapter.birthdayAdapter).size == 1) { "Adapter types are expected to be identical." }
